...
export default function InboundInboxPage() {
  ...
  if (!authenticated) {
    return (
      <div style={{ minHeight: "100vh", background: LIGHT_BG, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 420, maxWidth: "100%", background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 28 }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "10px 12px", border: `1px solid ${BORDER}`, borderRadius: 8, marginBottom: 10 }}
          />
          <button
            onClick={handleLogin}
            disabled={!apiKey.trim()}
            style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "none", background: BLUE, color: WHITE, fontWeight: 700, cursor: "pointer" }}
          >
            Sign In
          </button>
        </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: LIGHT_BG, fontFamily: "'Lato', system-ui, sans-serif" }}>
      {toast && (
        <div style={{ position: "fixed", top: 16, right: 16, padding: "10px 14px", background: NAVY, color: WHITE, borderRadius: 8, zIndex: 1000 }}>
          {toast}
        </div>
      )}

      <div style={{ background: NAVY, color: WHITE, padding: "18px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
        <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Link href="/admin/followups" style={{ color: "#8BA3CF", textDecoration: "none", fontSize: 12 }}>Follow-ups</Link>
          <Link href="/admin/analytics" style={{ color: "#8BA3CF", textDecoration: "none", fontSize: 12 }}>Analytics</Link>
          <button
            onClick={() => {
              setAuthenticated(false);
              sessionStorage.removeItem("admin_api_key");
            }}
            style={{ background: "transparent", color: "#8BA3CF", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 6, padding: "6px 10px", fontSize: 12, cursor: "pointer" }}
          >
            Sign Out
          </button>
        </div>
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: 20 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10, marginBottom: 14 }}>
          {[
            ...
            <div key={card.label} style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center", marginBottom: 16 }}>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ padding: "8px 10px", borderRadius: 8, border: `1px solid ${BORDER}` }}>
          ...
          <select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)} style={{ padding: "8px 10px", borderRadius: 8, border: `1px solid ${BORDER}` }}>
          ...
          <button onClick={fetchInquiries} style={{ padding: "8px 10px", borderRadius: 8, border: `1px solid ${BORDER}`, background: WHITE, cursor: "pointer" }}>
            Refresh
          </button>
        ...
        {!loading && inquiries.length === 0 && (
          <div style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 28, textAlign: "center", color: SUB }}>
        ...
        {inquiries.map((inquiry) => {
          ...
          return (
            <div key={inquiry.id} style={{ background: WHITE, border: `1px solid ${BORDER}`, borderRadius: 10, marginBottom: 12, overflow: "hidden" }}>
              <button
                onClick={() => setExpandedId(expanded ? null : inquiry.id)}
                style={{ width: "100%", textAlign: "left", border: "none", background: "transparent", padding: 14, cursor: "pointer" }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: STATUS_COLORS[inquiry.status], background: `${STATUS_COLORS[inquiry.status]}20`, padding: "3px 8px", borderRadius: 999 }}>
                      {STATUS_LABEL[inquiry.status]}
                    </span>
                    <span style={{ fontSize: 10, fontWeight: 700, color: NAVY, background: "#E9EFF7", padding: "3px 8px", borderRadius: 999 }}>
                ...
                <div style={{ borderTop: `1px solid ${BORDER}`, padding: 14 }}>
                  ...
                  {inquiry.message && (
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: 13, color: NAVY, background: LIGHT_BG, border: `1px solid ${BORDER}`, borderRadius: 8, padding: 10 }}>
                        {inquiry.message}
                      </div>
                    </div>
                  )}
                  ...
                  {inquiry.transcript && (
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ fontSize: 12, lineHeight: 1.6, color: NAVY, background: LIGHT_BG, border: `1px solid ${BORDER}`, borderRadius: 8, padding: 10, maxHeight: 180, overflowY: "auto" }}>
                        {inquiry.transcript}
                      </div>
                    </div>
                  )}
                  ...
                  <textarea
                    ...
                    rows={3}
                    style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: `1px solid ${BORDER}`, marginBottom: 8 }}
                  />
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button
                      ...
                      disabled={saving}
                      style={{ padding: "8px 10px", borderRadius: 8, border: "none", background: BLUE, color: WHITE, fontWeight: 700, cursor: "pointer" }}
                    >
                      Mark In Progress
                    </button>
                    <button
                      ...
                      disabled={saving}
                      style={{ padding: "8px 10px", borderRadius: 8, border: "none", background: GREEN, color: WHITE, fontWeight: 700, cursor: "pointer" }}
                    >
                      Mark Responded
                    </button>
                    <button
                      ...
                      disabled={saving}
                      style={{ padding: "8px 10px", borderRadius: 8, border: "none", background: RED, color: WHITE, fontWeight: 700, cursor: "pointer" }}
                    >
                      Close
                    </button>
                  </div>
