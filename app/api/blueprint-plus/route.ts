// app/api/blueprint-plus/route.ts
// ------------------------------------------------------
// WunderBrand Blueprint+™ report generator
// 1. Fetches base snapshot assessment data
// 2. Generates AI-powered Blueprint+ report (multi-call pipeline)
// 3. Saves to brand_snapshot_plus_reports (with tier: blueprint_plus)
// 4. Returns reportId for redirect
//
// NOTE: Blueprint+ uses a 4-call AI pipeline and can take 2-4 minutes.
// The frontend should show a progress indicator during generation.
// ------------------------------------------------------
...
export async function POST(req: Request) {
  try {
    ...
    const guard = apiGuard(req, { routeId: "blueprint-plus-generate", rateLimit: AI_RATE_LIMIT, maxBodySize: 500_000 });
    ...
    logger.info("[Blueprint+ API] Starting AI generation (multi-call pipeline)", {
      businessName: assessmentData.businessName,
      hasBaseSnapshot: !!base_snapshot_report_id,
    });

    // ─── Generate AI report (multi-call pipeline) ───
    const generatedReport = await generateAIReport("blueprint_plus", assessmentData);
    ...
    logger.info("[Blueprint+ API] Report saved", {
      ...
      businessName: assessmentData.businessName,
      model: generatedReport.model,
      generationTime: generatedReport.generatedAt,
    });
    ...
    if (format === "pdf") {
      try {
        ...
        const pdfBuffer = await renderToBuffer(
          React.createElement(BlueprintPlusDocument, { data: generatedReport.content }) as any
        );
