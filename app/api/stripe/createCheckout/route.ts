...
export async function POST(req: Request) {
  ...
  const guard = await apiGuard(req, { routeId: "stripe-checkout", rateLimit: AUTH_RATE_LIMIT });
  ...
  try {
    ...
    const sessionParams: Stripe.Checkout.SessionCreateParams = {
      ...
      metadata: {
        ...
        user_id: userId ?? "",
        ...(smsOptedIn ? { sms_opted_in: "true", sms_optin_source: "stripe_createCheckout" } : {}),
        ...(emailMarketingOptedIn ? { email_marketing_opted_in: "true", email_marketing_optin_source: "stripe_createCheckout" } : {}),
