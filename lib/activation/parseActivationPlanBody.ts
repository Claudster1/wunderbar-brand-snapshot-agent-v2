export type ActivationBodySubsection = {
  id: string;
  title: string;
  content: string;
};
...
export type ActivationBodySection = {
  ...
  title: string;
  content: string;
  /** When the `##` block contains multiple `###` headings, each becomes its own card. */
  subsections?: ActivationBodySubsection[];
};
...
/**
 * When a `##` section body contains `###` headings, split into intro + subsection blocks
 * so the UI can render distinct cards (lead magnet, SEO pack, etc.).
 */
function splitMarkdownIntoH3Blocks(
  parentId: string,
  content: string,
): { intro: string; subsections: ActivationBodySubsection[] } | null {
  ...
  const subsections: ActivationBodySubsection[] = [];
  let subIdx = 0;
  ...
  while (i < lines.length) {
    ...
    subsections.push({
      id: `${parentId}-sub-${subIdx}-${slugPart(title)}`,
      title,
      content: body,
    });
    subIdx++;
...
/** Split any section body that contains `###` blocks into subsection cards (all activation plan shapes). */
function enrichAllSectionsWithNestedH3(sections: ActivationBodySection[]): ActivationBodySection[] {
  return sections.map((sec) => {
    const inner = splitMarkdownIntoH3Blocks(sec.id, sec.content);
    if (!inner) return sec;
    return {
      ...sec,
      content: inner.intro,
...
export function splitActivationBodyIntoSections(body: string): ActivationBodySection[] {
  ...
  if (hasH2) {
    const parts = trimmed
      ...
      .filter(Boolean);
    const out = parts
      .map((chunk, idx) => parseMarkdownChunk(chunk, idx, "h2"))
      .filter((x): x is ActivationBodySection => x !== null);
    if (out.length > 0) return enrichAllSectionsWithNestedH3(out);
  }
  ...
  if (hasH3) {
    const parts = trimmed
      ...
      .filter(Boolean);
    const out = parts
      .map((chunk, idx) => parseMarkdownChunk(chunk, idx, "h3"))
      .filter((x): x is ActivationBodySection => x !== null);
    if (out.length > 0) return enrichAllSectionsWithNestedH3(out);
  }
  ...
  return enrichAllSectionsWithNestedH3(out);
}
