/**
 * Server-side brand asset analysis (OpenAI vision / text).
 * Called from authenticated API routes and from report generation (no HTTP hop).
 */

import OpenAI from "openai";
import { supabaseServer } from "@/lib/supabase";
import { logger } from "@/lib/logger";
import type { BrandContextForAssets } from "@/lib/assetAnalysis";
...
function buildPillarContext(ctx: BrandContextForAssets): string {
  const parts: string[] = [];
...
function buildAnalysisPrompt(
  ...
  fileName: string,
  category: string,
  brandCtx: BrandContextForAssets
): string {
...
export type AnalyzeBrandAssetsResult = {
  analyzed: number;
  results: Array<{ id: string; file_name: string; analysis: unknown }>;
  message?: string;
};

/**
 * Analyze unanalyzed assets for a verified owner email + Blueprint tier.
 */
export async function analyzeBrandAssets(opts: {
  email: string;
  tier: string;
  brandContext?: BrandContextForAssets;
}): Promise<AnalyzeBrandAssetsResult> {
  const email = opts.email.trim().toLowerCase();
  const tier = opts.tier;

  if (!isBlueprintLogoTier(tier)) {
    return { analyzed: 0, results: [], message: "Invalid tier." };
  }

  const sb = supabaseServer();
  let ctx: BrandContextForAssets = { ...(opts.brandContext || {}) };
  ...
  if (!ctx.pillarScores) {
    try {
      const { data: report } = (await sb
        ...
        .limit(1)
        .single()) as {
        data: { pillar_scores: Record<string, number> | null; company_name: string | null } | null;
      };
  ...
  const { data: assets, error: fetchErr } = (await sb
    ...
    .eq("tier", tier)
    .is("analysis", null)
    .limit(maxAssetsForTier(tier) || 10)) as { data: AssetRow[] | null; error: unknown };

  if (fetchErr || !assets || assets.length === 0) {
    return { analyzed: 0, results: [], message: "No unanalyzed assets found." };
  }
  ...
  for (const asset of assets) {
    try {
      ...
      if (asset.file_type.startsWith("image/")) {
        const { data: fileData } = await sb.storage.from("brand-assets").download(asset.storage_path);
      ...
      logger.error("[Asset Analyze] Error analyzing asset", {
